...
team_token=constants.zhipu_header_authorization
headers = {
    ...
    """
        ...
        例如：
            ...
            {'上市日期': '1997-07-18',
             ...
             '首发募资净额': '77250.0'}
        """
    company_new_name = search_company_name_by_info("公司简称", company_name)  # 处理query中的公司简称
    if len(company_new_name) == 0:
        company_new_name = search_company_name_by_info("英文名称", company_name)  # 处理query中的公司的英文名称
        if len(company_new_name) == 0:
            result = get_company_info_http(company_name)
    else:
        result = get_company_info_http(company_new_name["公司名称"])
    return result


def get_company_info_http(company_name: Union[str, List[str]]) -> dict:
    url = constants.zhipu_url + '/law_api/get_company_info'
    data = {
...
def search_company_name_by_info(key: str, value: str) -> dict:
    ...
    """
    url = constants.zhipu_url + '/law_api/search_company_name_by_info'
...
def get_company_register(company_name: Union[str, List[str]]) -> dict:
    """
        ...
        """
    result = get_company_register_http(company_name)

    # if len(result) == 0:
    #     company_name = full_to_half(company_name)
    #     result = get_company_register_http(company_name)

    #     if len(result) == 0:
    #         company_name = half_to_full(company_name)
    #         result = get_company_register_http(company_name)

    return result


def get_company_register_http(company_name: Union[str, List[str]]) -> dict:
    url = constants.zhipu_url + '/law_api/get_company_register'
...
def search_company_name_by_register(key: str, value: str) -> dict:
    ...
    例如：
        ...
        {"公司名称": "广州发展集团股份有限公司"}
    """
    url = constants.zhipu_url + '/law_api/search_company_name_by_register'
...
def get_sub_company_info(company_name: Union[str, List[str]]) -> dict:
    """
        ...
        例如：
            ...
            {'上市公司关系': '子公司',
             ...
             '关联上市公司股票简称': '冠昊生物'}
        """
    result = get_sub_company_info_http(company_name)
    # if len(result) == 0:
    #     company_name = full_to_half(company_name)
    #     result = get_sub_company_info_http(company_name)
...
def get_sub_company_info_http(company_name: Union[str, List[str]]) -> dict:
    url = constants.zhipu_url + '/law_api/get_sub_company_info'
...
def search_company_name_by_sub_info(key: str, value: str) -> dict:
    """
    ...
    例如：
        ...
        [{'公司名称': '北昊干细胞与再生医学研究院有限公司'},
         ...
         {'公司名称': '冠昊生命健康科技园有限公司'}]
    """
    url = constants.zhipu_url + '/law_api/search_company_name_by_sub_info'
    ...
    """
        ...
        例如：
            ...
            {'判决结果': '一、南昌绿地申新置业有限公司于本判决生效之日起十五日内向上海澳辉照明电器有限公司支付本金1179104元。\n'
             ...
             '被告律师': '罗丽萍,公司员工\n李某某,江西豫章律师事务所律师\n蔡某某,江西豫章律师事务所实习律师'}
        """
    result = get_legal_document_http(case_num)
    # if len(result) == 0:
    #     case_num = full_to_half(case_num)
    #     result = get_legal_document_http(case_num)
    return result


def get_legal_document_http(case_num: Union[str, List[str]]) -> dict:
    url = constants.zhipu_url + '/law_api/get_legal_document'
...
def search_case_num_by_legal_document(key: str, value: str) -> dict:
    ...
    """
    url = constants.zhipu_url + '/law_api/search_case_num_by_legal_document'
